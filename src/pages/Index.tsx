    ...
    <div className="min-h-screen">
      ...
      <section className="relative h-[90vh] flex items-center justify-center overflow-hidden">
        ...
        <div className="relative z-10 text-center px-4 max-w-3xl animate-fade-in-up">
          <p className="text-primary font-medium tracking-[0.3em] uppercase text-sm mb-4">{t("hero.welcome")}</p>
      ...
      <section className="container mx-auto px-4 -mt-12 relative z-20">
        <div className="bg-card rounded-xl shadow-luxury p-6 flex flex-col md:flex-row gap-4 items-end">
          <div className="flex-1 w-full">
            <label className="text-xs font-medium text-muted-foreground mb-1 block">{t("search.checkIn")}</label>
          ...
          <div className="flex-1 w-full">
            <label className="text-xs font-medium text-muted-foreground mb-1 block">{t("search.checkOut")}</label>
            <div className="flex items-center gap-2 border border-border rounded-md px-3 py-2">
              <CalendarDays size={16} className="text-primary" />
              <input type="date" className="bg-transparent text-sm text-foreground w-full outline-none" />
            </div>
          ...
          <div className="flex-1 w-full">
            <label className="text-xs font-medium text-muted-foreground mb-1 block">{t("search.guests")}</label>
            <div className="flex items-center gap-2 border border-border rounded-md px-3 py-2">
              <Users size={16} className="text-primary" />
              <select className="bg-transparent text-sm text-foreground w-full outline-none">
                {[1, 2, 3, 4].map((n) => (
                  <option key={n} value={n}>{n} {t("search.guest")}{n > 1 ? "s" : ""}</option>
                ))}
              </select>
            </div>
          </div>
          <Button asChild className="gold-gradient border-0 px-8 w-full md:w-auto">
            <Link to="/rooms">{t("search.search")}</Link>
          </Button>
        </div>
      </section>
      ...
      <section className="container mx-auto px-4 py-24">
        <div className="text-center mb-12">
          ...
          <h2 className="font-display text-3xl md:text-4xl font-bold text-foreground">{t("rooms.title")}</h2>
        </div>
      ...
      <section className="bg-muted py-24">
        <div className="container mx-auto px-4">
          <div className="text-center mb-12">
            <p className="text-primary tracking-[0.2em] uppercase text-sm font-medium mb-2">{t("reviews.subtitle")}</p>
            <h2 className="font-display text-3xl md:text-4xl font-bold text-foreground">{t("reviews.title")}</h2>
          </div>
      ...
      <section className="container mx-auto px-4 py-24 text-center">
        <h2 className="font-display text-3xl md:text-4xl font-bold text-foreground mb-4">{t("cta.title")}</h2>
        <p className="text-muted-foreground mb-8 max-w-lg mx-auto">{t("cta.desc")}</p>
        <Button asChild size="lg" className="gold-gradient border-0 text-lg px-10 py-6">
          <Link to="/rooms">{t("cta.bookRoom")}</Link>
        </Button>
      </section>
    </div>
  );
